...
procedure TfrmSobre.SetMensagem(const Value: string);
begin
  FMensagem := Value;
  Self.LblStatus.Text := FMensagem;
end;

procedure TfrmSobre.SetModo(const Value: TSobreModo);
begin
  FModo := Value;
...
procedure TfrmSobre.Mostrar(const AModo: TSobreModo; const AMensagem: string);
begin
  Self.Modo     := AModo;
...
procedure TfrmSobre.Fechar;
begin
  Self.Visible := False;
end;

procedure TfrmSobre.RecMsg1Click(Sender: TObject);
begin
  OpenURL('https://corona.lmao.ninja');
end;

procedure TfrmSobre.RecMsg2Click(Sender: TObject);
begin
  OpenURL('https://corona.lmao.ninja/docs');
end;

procedure TfrmSobre.RecMsg3Click(Sender: TObject);
begin
  OpenURL('https://www.worldometers.info/coronavirus');
end;

procedure TfrmSobre.Rectangle2Click(Sender: TObject);
begin
